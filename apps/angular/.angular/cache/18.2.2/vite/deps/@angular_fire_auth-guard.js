import { Router } from './chunk-ECWJSQPH.js';
import './chunk-ZAWNPVTG.js';
import { Auth, user } from './chunk-C5PJ66XI.js';
import { VERSION } from './chunk-JHJIENA6.js';
...
import { map, of, pipe, switchMap, take } from './chunk-BO5GAICC.js';
...
var idTokenResult = switchMap((user2) =>
  user2 ? user2.getIdTokenResult() : of(null)
);
var emailVerified = map((user2) => !!user2 && user2.emailVerified);
var customClaims = pipe(
  idTokenResult,
...
var redirectUnauthorizedTo = (redirect) =>
  pipe(
    loggedIn,
    map((loggedIn2) => loggedIn2 || redirect)
  );
var redirectLoggedInTo = (redirect) =>
  pipe(
    loggedIn,
